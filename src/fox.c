...
#include "cblas.h"
/*
  Implementation of init_fox_mult_marix defined in fox.h
 */
void init_fox_mult_marix(int nb_proc_row, MPI_Comm *communicator, struct grid * grid, int rank){
...
void mult_fox_mpi(int n, 
  ...
  for(c = 0; c < grid->n; c++){
    ...
    MPI_Isend(Bkj_current, grid->n*grid->n, MPI_DOUBLE, grid->process_above, COMM_TAG_BKI, communicator, &bkj_send_request);
    MPI_Irecv(next_Bkj, grid->n*grid->n, MPI_DOUBLE, grid->process_under, COMM_TAG_BKI, communicator, &bkj_recv_request);
